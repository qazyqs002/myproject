...
# File:15、生成虚拟数据测试用的faker库.py
# Time:2021/11/15  10:51
# Author: yqs
"""
...
faker_1 = Faker("zh_CN")  # 创建一个faker_1 对象 括号中加参数为中文， 默认是英文

l_name = faker_1.name()
l_phone = faker_1.phone_number()
l_address = faker_1.address()
l_company = faker_1.company()
l_job = faker_1.job()
l_text = faker_1.text()

print(f"姓名：{l_name}")
print(f"电话：{l_phone}")
print(f"地址：{l_address}")
print(f"公司：{l_company}")
print(f"工作：{l_job}")
print(f"随机文本：{l_text}")
...
fake.pylist(nb_elements=10, variable_nb_elements=True)  # Python列表
# [   589,
...
fake.pyset(nb_elements=10, variable_nb_elements=True,)  # Python集合
...
fake.pystruct(count=10,)  # Python结构
...
fake.pytuple(nb_elements=10, variable_nb_elements=True,)  # Python元组
